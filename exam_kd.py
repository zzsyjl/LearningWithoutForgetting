...
from exam_kd_trainer import train
from exam_kd_trainer import accuracy
...
def main(args):
    ...
    # save the data of previous tasks
    data_list = []
    ...
    for step in range(num_iter):
        ...
        test_loader = DataLoader(BatchData(test_x, test_y, transforms_test),
                                 batch_size=args.eval.batch_size, shuffle=False, num_workers=args.dataset.num_workers)
        ...
        if step == 0:
            ...
            previous_train_x, previous_train_y = zip(*data_list)
            previous_train_loader = DataLoader(BatchData(previous_train_x, previous_train_y, transforms_train),
                                               batch_size=args.train.batch_size, shuffle=True, num_workers=args.dataset.num_workers)
            train(model, train_loader, step, device, args, previous_train_loader)

        save_previous_data(data_list, train_x, train_y, 500)
